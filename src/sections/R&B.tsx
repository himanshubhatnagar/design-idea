...
import { 
  ...
  ChevronRight, Database, FileText, Briefcase, 
  ShieldCheck, Wrench, BadgeCheck
} from "lucide-react";
...
const RollingNumber = ({ value, suffix = "" }: { value: string; suffix?: string }) => {
  ...
  useEffect(() => {
    return springValue.on("change", (latest) => {
      setCurrent(Math.floor(latest).toLocaleString());
    });
  }, [springValue]);
...
const capabilityData = [
  {
    ...
    icon: Search,
    services: ["Market Analysis", "Competitor Intelligence", "Financial Benchmarking", "Target Screening"],
    ...
    tools: [
      ...
      { name: "Capital IQ", logo: "https://logo.clearbit.com/spglobal.com" },
      { name: "Thomson Reuters", logo: "https://logo.clearbit.com/thomsonreuters.com" }
    ],
    // OPTIONAL: Certifications
    certifications: ["ISO 9001", "PMP Certified Team", "Lean Six Sigma"]
  },
  {
    id: "account-mgmt",
    title: "Account Management Services",
    icon: Briefcase,
    services: ["CRM Management", "Pipeline Tracking", "Revenue Reporting"],
    sectors: ["Global Markets", "Enterprise Sales"],
    metrics: [
      { label: "Active Accounts", val: "120", sub: "Managed Monthly", suffix: "+", icon: Database, type: "number" },
      { label: "Data Integrity", val: "99", sub: "Accuracy Rate", suffix: "%", icon: ShieldCheck, type: "number" },
      { label: "Efficiency", val: "450", sub: "Process Hours Saved", suffix: "+", icon: Zap, type: "number" },
    ],
    efficiency: { savings: "12500", gains: "24%" },
    award: "Best-in-class Process Optimization",
    // This section has NO tools or certifications - UI will adjust
  }
];
          ...
          <div className="lg:w-80 bg-white/[0.02] border-r border-white/5 p-8">
            ...
            <nav className="flex flex-col gap-2">
              {capabilityData.map((item) => (
                ...
                >
                  <item.icon size={18} />
                  <span className="text-xs font-bold uppercase tracking-wider leading-tight">{item.title}</span>
                </button>
              ))}
            </nav>
          </div>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-12 mb-12">
                  <div>
                    <h4 className="text-xs font-black text-blue-500 uppercase tracking-[0.4em] mb-6">Service Portfolio</h4>
                  ...
                  <div className="bg-white/[0.03] p-6 rounded-3xl border border-white/5">
                    <p className="text-[10px] text-gray-500 font-bold uppercase mb-4 tracking-widest text-left">Sectors</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                  {activeData.metrics.map((m, i) => (
                    <div key={i} className="bg-white/[0.02] border border-white/5 p-8 rounded-3xl">
                      ...
                      <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest text-left">{m.label}</p>
                ...
                {/* 3. CONDITIONAL CARDS: Tools & Certifications */}
                {(activeData.tools || activeData.certifications) && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    {activeData.tools && (
                      <div className="bg-[#0f1115] border border-white/5 p-6 rounded-3xl">
                        <div className="flex items-center gap-2 mb-4 text-blue-400">
                          <Wrench size={16} />
                          <span className="text-[10px] font-bold uppercase tracking-widest">Tools Supported</span>
                        </div>
                        <div className="flex flex-wrap gap-6 items-center">
                          {activeData.tools.map((tool, i) => (
                            <img key={i} src={tool.logo} alt={tool.name} title={tool.name} className="h-6 opacity-60 grayscale hover:grayscale-0 hover:opacity-100 transition-all" />
                          ))}
                        </div>
                      </div>
                    )}
                      ...
                      <div className="bg-[#0f1115] border border-white/5 p-6 rounded-3xl">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {activeData.certifications.map((cert, i) => (
                            <span key={i} className="px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 rounded-full text-[10px] font-bold">
                              {cert}
                            </span>
                          ))}
                        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                  <div className="lg:col-span-2 bg-gradient-to-br from-blue-600/10 to-transparent p-8 rounded-[2rem] border border-blue-500/10">
                    ...
                    <div className="flex justify-between">
                      <div>
                        ...
                        <p className="text-[10px] text-blue-400 font-bold uppercase mt-1 text-left">Hrs Saved</p>
                      </div>
                      <div>
                        <div className="text-3xl font-black"><RollingNumber value={activeData.efficiency.gains} suffix="%" /></div>
                        <p className="text-[10px] text-blue-400 font-bold uppercase mt-1 text-left">Gain</p>
                      </div>
                    </div>
                  </div>
                  <div className="lg:col-span-3 bg-white/[0.02] p-8 rounded-[2rem] border border-white/5 relative overflow-hidden group">
                    ...
                    <span className="text-xs font-black text-blue-500 uppercase tracking-widest block mb-4 italic text-left">Achievement</span>
                    <p className="text-sm text-gray-300 leading-relaxed font-medium text-left">{activeData.award}</p>
                  </div>
                </div>
              </motion.div>
            </AnimatePresence>
